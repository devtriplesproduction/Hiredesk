"use client";
import type { Candidate, Role } from "../src/types";
import { makeCandidate, detectBestRole, scoreCandidateFromText, SKILLS_POOL } from "../src/lib/data";
import { extractTextAndMetaFromPDF, PDFTextLine } from "../src/lib/utils/pdf";
 ...
 */
export function extractNLPPersonName(text: string): string {
 ...
 */
export function extractPDFHeadingName(firstPageLines: PDFTextLine[], transformations?: string[]): { headings: string[]; multilineJoined?: string } {
 ...
 */
export function extractEmailSenderName(sender?: string): string {
  if (!sender) return "";
 ...
 */
export function extractFilenameName(filename: string, transformations?: string[]): string {
 ...
 */
export function extractEmailSubjectName(subject?: string): string {
  if (!subject) return "";
  const patterns = [
 ...
 */
export function extractSignatureName(text: string): string {
 ...
 */
export function extractLinkedInURLName(text: string): string {
 ...
 */
export function extractOCRNameFallback(ocrText: string): string {
  if (!ocrText) return "";
...
export function extractEmploymentStatus(text: string): {
...
export function extractGender(text: string, name: string): string {
...
export function extractSkills(text: string, roleId: string): string[] {
  const lower = text.toLowerCase();
